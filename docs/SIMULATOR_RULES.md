...
| 精英 Huge | 怪物 HP +100% | `qmodifiers.json.huge.mHp=1` | B | 同上 |
...
| 精英 Epic | HP ×1.5、攻击 ×1.25、基础暴击率 ×1.5、回避 +0.1 | `epic.mHp=.5, mDmg=.25, mCrit=.5, mEva=.1` | B | 同上 |
...
| 限时怪物攻击/暴击 | `TimedMonsterModifier` 仅在 `round <= duration` 生效 | `xo` 把带 `duration` 的 `mDmg/mCrit` 组为 timed bonuses；`Lo` 用 `round <= duration` 汇总 | A | `timed_damage_and_threat_boundaries_match_bundle_helpers`；高级黄金 fixture |
| 每回合怪物增伤 | `MonsterDamagePerRound`：`(1 + delta × max(0, round-1)) × (1 + timedDelta)`，仅单体 | `Ho` 公式及回合日志“AOE 不受影响” | A | 同上精确断言第 1、3 回合 |
| AOE | `AreaAttack`：存活单位大于 1 时按概率触发；所有存活单位独立闪避；伤害为普通预计算伤害 × `damageRatio` 后 `ceil`，不暴击、不吃单体回合增伤 | `Ap` 的 `Math.random()<aoeChance && fightersAlive>1` 分支及 `Math.ceil(fighterDamageTaken * aoeRatio)` | A | 高级黄金 fixture；确定性测试覆盖多单位状态 |
