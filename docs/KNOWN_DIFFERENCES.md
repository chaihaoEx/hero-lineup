...
| 泰坦塔词条 | 已实现本地目录、名称/说明/图标、层/阶筛选、任务上限、family 互斥、随机、墓穴楼层与祝福灯笼；怪物 HP/攻击/回避/暴击/暴伤、持续攻击/暴击、每回合伤害进入模拟 | 线上还逐条处理英雄职业修正、屏障额外值、怪物 AOE、回复、禁元素导致的队伍即时剔除等字段 | UI 状态与主要怪物修正一致；列出的特殊字段仍有偏差 |
